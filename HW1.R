...
#To see the help file associated with a function, simply type "?" followed by the function name. E.g.:
?getwd

#You can set the default working directory using the function "setwd" like so on a PC:
setwd("C:\Users\mmkeller\Documents\RClass")	#doesn't work on MAC, and won't work on PC unless you already have a folder, "RClass" in your Documents folder	

#Or like so on a MAC:
setwd("/Users/mmkeller/Documents/RClass") #doesn't work on PC, and won't work on MAC unless you already have a folder, "RClass" in Documents and your user name is mmkeller (which I doubt)
setwd("~/Documents/RClass") #is equivalent to above. "~" is shortcut for "/Users/<username>"


# If you run the above, the "RClass" folder on your computer is the new default folder for saving and importing data for this session. This is where R looks by default to read and write files. Obviously, you can change the working directory whenever you'd like in a session. 
...
#Some people are not used to knowing where their files and folders *actually* are on their computer. It's helpful to know the full path of your files/folders. To do so, see:
#For PC: https://www.laptopmag.com/articles/show-full-folder-path-file-explorer
#For MAC: https://www.lifewire.com/use-macs-hidden-finder-path-bar-2260868
...
# (d) Create a folder on your computer that will be your actual working directory for the rest of this session. Change your working directory to this folder.
# path to folder is: '~/Desktop/psyc5541'
...
#HW Problem 2 ----------------------------------------------------------
# (a) Create an object called "hw2a" that is the square root of (19 divided by 3)
hw2a <- sqrt(19/3)
...
#There are many many more classes (thousands) than modes (maybe a dozen, and just a few that you see often: numeric, character, logical, factor, and function). Functions often need the class of an object in order to know how to deal with the object. I know that the distinction between mode & class isn't obvious yet. It'll get more clear once we get into different types of objects below.
...
# Remove object x3 (changed from x4)
remove(x3)				
ls()
...
(hw3c <- (((hw.vector <=80 & hw.vector >= 30)*5) +(hw.vector >= 80 | hw.vector <= 30)*-99))
...
(hw3d <- hw.vector[hw.vector < 70 & hw.vector > 30]) 
...
size <- 2500
normdist <- rnorm(size, mean = 100, sd =15)
bidist <- rbinom(size, 1, .1)
bidist[bidist == 1] <- 30
binomdist <-  normdist + bidist
binombinarydist <- rbinom(2500, 1, .25)

hw4a <- data.frame(col1 = normdist, binomdist, binombinarydist)
names(hw4a) <- c('normdist','dist_outliers','binary_dist')

# (b) Plot a histogram of the first two variables. Then make a scatterplot of the first two variables against each other. See ?hist and?plot
hist(normdist)
hist(binomdist)
plot(normdist,binomdist)
...
mean(normdist);median(normdist);var(normdist);summary(normdist)
mean(binomdist);median(binomdist);var(binomdist);summary(normdist)
sum(binombinarydist ==1)/length(binombinarydist)
...
binary_logical <- binombinarydist == 1
norm_indexed <- normdist[binary_logical]
binom_indexed <- binomdist[binary_logical]
hw4d1 <- c(mean(norm_indexed),median(norm_indexed),var(norm_indexed))
hw4d2 <- c(mean(norm_indexed),median(norm_indexed),var(norm_indexed))
...
hw4a_random <- hw4a[sample(nrow(hw4a), 50), ]
hw4a_random_outliers <- hw4a_random[2]
hw4e <- c(colMeans(hw4a_random_outliers), (apply(hw4a_random_outliers, 2, sd)))
...
rand <- hw4a[sample(nrow(hw4a), 50), ]
col <- rand[1]
sum_stats <- c(colMeans(col), (apply(col, 2, sd)))
hw4f <- matrix(sum_stats,1,2, byrow=TRUE)

rand <- hw4a[sample(nrow(hw4a), 50), ]
col <- rand[1]
sum_stats <- c(colMeans(col), (apply(col, 2, sd)))
hw4f <- rbind(hw4f,sum_stats)
...
write.table(hw4f,file="/Users/jacklovell/Desktop/psyc5541/my.montecarlo.hw.txt",quote=FALSE,col.names=TRUE,row.names=FALSE)
...
save(x12,x11a,file="threeobjects.RData")  
