...
str(hw1ca);str(hw1cb)   
ans2c <- "the first uses data.frame which allows for different modes of columns. The second first converts to a matrix, which must all have the same mode, and converts them to the lowest mode (here, characters)"
...
hw1e <- data.frame(score=score,country=country,stringsAsFactors=TRUE)
...
(ans1g <- sum(hw1ca$score,na.rm=TRUE))
...
(ans1h <- sum(as.numeric(as.character(hw1cb$score)),na.rm=TRUE))
ans1h==ans1g
...
(hw1j <- mean(hw1e$score[hw1j.tf],na.rm=TRUE))
...
My.CSV.data <- read.table("MyData.csv",header=T,sep=",")
...
DAT2$heart <- as.factor(DAT2$heart)
str(DAT2)
